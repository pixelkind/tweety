...
class Tween {
  ...
  set pValue(value) {
    ...
    if (this._aValue < 1) {
      ...
      this._sValue = (this._pValue / (Math.PI * 2)) * Math.asin(1 / this._aValue);
    }
  }
  ...
  update(timestamp) {
    if ((this.currentTime >= this.delay && this.speed > 0) || this.speed < 0) {
      ...
      for (let key in this.params) {
        this.target[key] = this.params[key].from + value * (this.params[key].to - this.params[key].from);
      }
  ...
  easeInElastic(t) {
    ...
    return -(postfix * Math.sin(((t - this._sValue) * (Math.PI * 2)) / this.pValue));
  }
  ...
  easeOutElastic(t) {
    ...
    return Math.pow(2, -10 * t) * Math.sin(((t - this._sValue) * (Math.PI * 2)) / this.pValue) + 1;
  }
  ...
  easeInOutElastic(t) {
    ...
    if (t < 1) {
      let postfix = Math.pow(2, 10 * (t -= 1));
      return -0.5 * (postfix * Math.sin(((t - this._sValue) * (Math.PI * 2)) / this.pValue));
    } else {
      let postfix = Math.pow(2, -10 * (t -= 1));
      return postfix * Math.sin(((t - this._sValue) * (Math.PI * 2)) / this.pValue) * 0.5 + 1;
    }
  }
